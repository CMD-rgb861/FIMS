...
class ReportsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $unitFilter = $request->query('unit_filter', null);
        ...
        $units = [];
        if ($currentUser->isAdmin() || $currentUser->isDean()) {
            $unitsQuery = Unit::query()
                ->select('id', 'name')
                ->orderBy('name');

            if ($currentUser->isDean()) {
                $collegeId = $currentUser->dean?->college_id ?? $currentUser->college_id;

                if ($collegeId !== null) {
                    $unitsQuery->where('department_id', $collegeId);
                }
            }

            $units = $unitsQuery->get()->toArray();
        }

        $facultyEvaluations = $this->getLocalFacultyUsers($currentUser, $termId, $facultySearch, $unitFilter);

        $allSubmissionsQuery = SupervisorEvaluationSubmission::query()
            ->where('user_id', $currentUser->id);

        if ($termId) {
            $allSubmissionsQuery->where('term_id', $termId);
        }
        ...
        $facultyIdNos = collect($facultyEvaluations)
            ->pluck('id_no')
            ->filter()
            ->unique()
            ->values()
        ...
        $sefAveragesByIdNo = $this->getFacultySefAverages($facultyIdNos, $termId);
        $batchSetRatings = $this->getBatchFacultySetRatings($facultyIdNos, $termId);
        ...
        // Get paginated IDs for batch loading (optimization)
        $paginatedIds = $paginatedFaculty->pluck('id_no')->filter()->values()->all();
        
        // Re-fetch ratings for only paginated items to reduce data transfer
        $paginatedSetRatings = $batchSetRatings;
        if (!empty($paginatedIds)) {
            $paginatedSetRatings = $this->getBatchFacultySetRatings($paginatedIds, $termId);
        }
        ...
        $facultyList = $paginatedFaculty
            ->map(function ($faculty, $index) use ($evaluatedInstructors, $sefAveragesByIdNo, $paginatedSetRatings, $offset, $termId) {
                ...
                if ($subjectsCount > 0) {
                    // Use batch-loaded SET ratings - NO DATABASE QUERY HERE!
                    $overallSetRating = $paginatedSetRatings->get($faculty['id_no']);

                    $averageSefRating = $sefAveragesByIdNo->get($faculty['id_no'] ?? null);
                    if ($averageSefRating !== null) {
                        $overallSefRating = round($averageSefRating, 2);
                    }
                }
        ...
        $facultyListAll = $facultyCollection
            ->map(function ($faculty, $index) use ($evaluatedInstructors, $sefAveragesByIdNo, $batchSetRatings, $termId) {
                ...
                if ($subjectsCount > 0) {
                    $overallSetRating = $batchSetRatings->get($faculty['id_no']);
                    $averageSefRating = $sefAveragesByIdNo->get($faculty['id_no'] ?? null);
                    if ($averageSefRating !== null) {
                        $overallSefRating = round($averageSefRating, 2);
                    }
    ...
    public function faculty(Request $request, string $instructor)
    {
        ...
        if ($courseCodes->isNotEmpty()) {
            $latestSubmissionIds = SupervisorEvaluationSubmission::query()
                ->where('user_id', $currentUser->id)
                ->where('instructor_id_no', $facultyMeta['id_no'])
        ...
        $overallSefRating = null;
        $sefAveragesByIdNo = $this->getFacultySefAverages([$facultyMeta['id_no']], $selectedSchoolYear);
        $averageSefRating = $sefAveragesByIdNo->get($facultyMeta['id_no'] ?? null);
        if ($averageSefRating !== null) {
            $overallSefRating = round($averageSefRating, 2);
        }
     ...
     */
    private function getLocalFacultyUsers($user, ?int $selectedSchoolYearId = null, string $search = '', $unitFilter = null): array
    {
        if (!$user) {
            return [];
        }
        ...
        if ($user->isAdmin() || $user->isDean()) {
            if ($unitFilter && is_numeric($unitFilter)) {
        ...
        */
        ...
        return $users->map(function ($localUser) use ($subjectCountsByIdNo) {
            ...
            return [
                ...
                'unit_id' => $localUser->unit_id, // ADD THIS TOO
            ];
    ...
    private function getFacultySefAverages(array $facultyIdNos, ?int $termId = null)
    {
    ...
    private function facultyIndex(Request $request)
    {
        ...
        $averageSefRating = $this->getFacultyReceivedSefRating($currentUser->id_no, $termId);
        ...
        $reportsProps = $this->commonInertiaProps($currentUser, [
            'reportSummary' => [
                ...
                [
                    ...
                    'value' => $averageSefRating !== null
                        ? number_format($averageSefRating, 2) . '%'
                        : 'N/A',    
    ...
    private function getBatchFacultySetRatings(array $facultyIdNos, ?int $termId = null): \Illuminate\Support\Collection
    {
        ...
        try {
            // Get all subjects with their student counts and avg ratings per faculty
            ...
            // Calculate weighted average per faculty (same logic as getFacultyOverallSetRating)
            $facultyTotals = [];
            foreach ($subjects as $subject) {
                $idNo = $subject->id_no;
